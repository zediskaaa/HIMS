...
<section class="py-4 first:pt-0" x-data="{ enabled: @js((bool) $user->sms_mfa_enabled), original: @js((bool) $user->sms_mfa_enabled) }"
         x-on:open-modal.window="if ($event.detail === 'configure-sms-mfa') enabled = original">
    ...
    <x-ui.modal name="configure-sms-mfa" :title="__('SMS Authentication')" maxWidth="md">
        ...
        <p class="mb-4 text-sm leading-5 text-neutral-600 dark:text-neutral-300">SMS codes are sent to your registered mobile number. Keep that number current to receive sign-in codes.</p>
