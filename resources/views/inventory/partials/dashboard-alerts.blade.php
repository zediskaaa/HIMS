        ...
        @forelse ($activeAlerts->take(3) as $alert)
            @php
                $severityClasses = match ($alert->severity) {
                    \App\Enums\AlertSeverity::Critical => 'bg-danger-50 text-danger-700 ring-danger-600/20',
                    \App\Enums\AlertSeverity::Warning => 'bg-warning-50 text-warning-700 ring-warning-600/20',
            ...
            <a href="{{ route('inventory.alerts') }}" class="flex items-start gap-3 px-4 py-2.5 transition-colors hover:bg-neutral-50">
                <span class="mt-1.5 h-2 w-2 shrink-0 rounded-full {{ $alert->severity === \App\Enums\AlertSeverity::Critical ? 'bg-danger-500' : ($alert->severity === \App\Enums\AlertSeverity::Warning ? 'bg-warning-500' : 'bg-primary-500') }}" aria-hidden="true"></span>
                <span class="min-w-0 flex-1">
                    <span class="block truncate text-xs font-semibold text-neutral-900">{{ $alert->item?->name ?? 'Unknown item' }}</span>
                    <span class="mt-0.5 block truncate text-[11px] text-neutral-500">
                        {{ $alert->type->label() }}
                        &middot; {{ number_format((int) ($alert->item?->quantity_on_hand ?? $alert->current_value)) }} on hand
                        @if ($alert->location)
                            &middot; {{ $alert->location->name }}
                        @endif
                    </span>
                </span>
