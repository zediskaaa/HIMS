    ...
    <div x-data="demandForecastDashboard({{ Js::from($dashboardForecastConfig) }})"
        ...
        <div class="grid grid-cols-1 gap-3.5 lg:grid-cols-12">
            ...
            <section class="min-w-0 overflow-hidden rounded-xl border border-neutral-200 bg-white shadow-xs lg:col-span-7 xl:col-span-8 dark:border-neutral-800 dark:bg-neutral-900"
                ...
                <div class="p-3">
                    {{-- 1. Loading / No-Data Skeleton (Exact Pic 2 layout) --}}
                    <div x-show="!isSuccess() || isLoading()" aria-busy="true">
                        <x-ui.forecast-chart-skeleton :chart-only="true" />
                    </div>

                    {{-- 2. Populated Chart Canvas --}}
                    <div x-show="isSuccess() && !isLoading() && hasChartData()" x-cloak class="min-w-0 select-none">
                    ...
                    {{-- 4. Error State --}}
                    <div x-show="isError()" x-cloak class="rounded-xl border border-danger-200 bg-danger-50/70 p-6 text-center dark:border-danger-900/60 dark:bg-rose-950/20">
                        <x-ui.icon name="exclamation-triangle" class="mx-auto h-7 w-7 text-danger-600 dark:text-rose-400" />
                        <h4 class="mt-2 text-sm font-semibold text-danger-900 dark:text-rose-200">Failed to load forecast data</h4>
                        <p class="mt-1 text-xs text-danger-700 dark:text-rose-300">The forecast service could not complete your request. Please try again.</p>
                        <button type="button" x-on:click="retryForecast()" class="mt-3 inline-flex items-center gap-1.5 rounded-md bg-danger-600 px-3 py-1.5 text-xs font-semibold text-white shadow-sm hover:bg-danger-500 dark:bg-rose-600 dark:hover:bg-rose-500">
            ...
            <section class="flex flex-col justify-between rounded-xl border border-neutral-200 bg-white p-4 shadow-xs dark:border-neutral-800 dark:bg-neutral-900 lg:col-span-5 xl:col-span-4 min-w-0">
                <div>
                    ...
                    <div class="mt-3 space-y-3">
                        {{-- 1. Loading Skeleton --}}
                        <div x-show="isLoading()" aria-busy="true">
                            <x-ui.forecast-insight-skeleton />
                        </div>

                        {{-- 2. Success Content --}}
                        <div x-show="isSuccess()" x-cloak class="space-y-3">
                        ...
                        <div x-show="isError()" x-cloak class="rounded-lg border border-danger-200 bg-danger-50/60 p-4 text-center text-xs text-danger-700 dark:border-danger-900/60 dark:bg-rose-950/20 dark:text-rose-300">
                            Unable to calculate clinical insights.
                        </div>
                    </div>
                </div>
